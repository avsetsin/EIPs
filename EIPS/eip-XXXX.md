...
discussions-to: https://ethereum-magicians.org/t/eip-xxxx-withdrawal-credentials-preregistration/XXXXX
status: Draft
...
| `VALIDATOR_PREREGISTRATIONS_LIMIT`                   | `uint64(2**22)` (= 4,194,304) | Maximum stored preregistrations in beacon state        |
...
Each preregistration request must appear in the EIP-7685 requests list in the exact order returned by `dequeue_preregistration_requests()`.
...
Without early BLS verification, an attacker could consume a preregistration by depositing with matching WC but an invalid signature, stripping protection from the pubkey. This is not a new pattern — [EIP-7732](./eip-7732.md) already verifies deposit BLS signatures at block time for builder routing. Preregistration applies the same approach to the narrow subset of deposits targeting preregistered pubkeys.
...
`VALIDATOR_PREREGISTRATIONS_LIMIT = 4,194,304` (2^22) provides headroom beyond the theoretical maximum throughput during the expiry window (`VALIDATOR_PREREGISTRATION_EXPIRY_SLOTS × MAX_VALIDATOR_PREREGISTRATION_REQUESTS_PER_PAYLOAD` = 2^18 × 4 = 2^20). The 4x margin exists because expiry is measured against `finalized_slot`: during extended non-finality, the finalized checkpoint stops advancing, so preregistrations stop expiring — but new preregistrations continue to arrive. The extra capacity absorbs this accumulation until finality resumes and the expiry sweep clears the backlog. Under normal operation the list remains near zero — preregistrations are consumed by matching deposits within hours. The fee mechanism makes sustained saturation economically prohibitive. Worst-case state size is ~369 MB (88 bytes × 4,194,304 entries).
...
An adversary could attempt to strip preregistration protection by depositing with matching WC but an invalid BLS signature. This EIP prevents this attack by verifying the deposit's BLS signature in `process_deposit_request` before consuming the preregistration. Only deposits with both matching WC and a valid BLS signature can consume a preregistration. Invalid-signature deposits are silently rejected without affecting the preregistration.

### Builder routing interaction (EIP-7732)

When [EIP-7732](./eip-7732.md) (ePBS) builder routing is active, deposits with `0x03` (builder) WC are routed directly to the builder registry, bypassing the standard pending deposit flow. The preregistration WC check must precede builder routing in `process_deposit_request` to prevent bypass.

### DoS and state growth

The EIP-1559-style fee mechanism is the primary spam defense. At sustained maximum throughput (4 per block vs target of 1), excess increases by 3 per block and the fee grows as `e^(excess/17)` wei. The fee exceeds 1 ETH after ~235 blocks (~47 minutes) of sustained maximum rate.

Under normal operation, the state list stays near zero (preregistrations consumed within hours). The 4,194,304 entry limit (~369 MB) provides defense-in-depth with headroom for non-finality periods, bounded by the expiry mechanism.
